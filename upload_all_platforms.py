...
def generate_caption(phrases, category, platform="vk"):
    ...
    caption_lines = [
        f"🇷🇺 Английский с Kregg! 🇬🇧",
        f"",
        f"📚 Тема: {category}",
        f"",
        f"🎯 Учим английский легко! Сегодняшние фразы:",
        f""
    ]
    ...
    for i, phrase in enumerate(phrases[:5]):
        ...
        caption_lines.append(f"   🔊 {phrase.get('english_cyrillic', '')}")
    ...
    caption_lines.extend([
        ...
        f"🔔 Подпишитесь на ежедневные уроки английского!",
        f"",
        f"📖 Как читать:",
...
def upload_to_all_platforms(video_path, caption, category, phrases=None):
    ...
    print("\n" + "="*80)
    print("🚀 ENGLISH WITH KREGG - MULTI-PLATFORM UPLOAD")
    print("="*80)
...
def main():
    ...
    print("\n" + "="*80)
    print("🇷🇺 ENGLISH WITH KREGG - AUTOMATED UPLOAD 🇬🇧")
    print("="*80)
