...
FALLBACK_PHRASES = {
    "Motivation": [
        {"english": "Every day, a new chance", "russian": "Каждый день, новый шанс", "english_cyrillic": "Эври дэй, э нью чанс"},
        {"english": "You are, stronger than you think", "russian": "Ты сильнее, чем думаешь", "english_cyrillic": "Ю ар, стронгер зэн ю синк"},
        {"english": "Small steps, big changes", "russian": "Маленькие шаги, большие изменения", "english_cyrillic": "Смол стэпс, биг чейнджес"},
        {"english": "Keep moving, keep growing", "russian": "Продолжай двигаться, продолжай расти", "english_cyrillic": "Кип мувинг, кип гроуинг"},
        {"english": "Your potential, is limitless", "russian": "Твой потенциал, безграничен", "english_cyrillic": "Ёр потеншл, из лимитлесс"},
    ],
    "Love": [
        {"english": "Love is, the answer", "russian": "Любовь, это ответ", "english_cyrillic": "Лав из, зе ансэ"},
        {"english": "Follow your, heart always", "russian": "Всегда следуй, за сердцем", "english_cyrillic": "Фоллоу ёр, харт олвэйз"},
        {"english": "Every heart, has a song", "russian": "У каждого сердца, есть песня", "english_cyrillic": "Эври харт, хэз э сонг"},
    ],
    "Success": [
        {"english": "Hard work, pays off", "russian": "Тяжелый труд, окупается", "english_cyrillic": "Хард вёрк, пэйс оф"},
        {"english": "Focus on, your goals", "russian": "Сосредоточься, на своих целях", "english_cyrillic": "Фокус он, ёр гоулз"},
    ],
}
...
def get_fallback_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    if not fallback:
        fallback = [
            {"english": "Learn something, new every day", "russian": "Учись чему-то новому каждый день", "english_cyrillic": "Лёрн самсинг, нью эври дэй"},
            {"english": "Practice makes, progress", "russian": "Практика ведёт, к прогрессу", "english_cyrillic": "Прэктис мэйкс, прогресс"},
            {"english": "Keep going, you can do it", "russian": "Продолжай, у тебя получится", "english_cyrillic": "Кип гоинг, ю кэн ду ит"},
        ]
    
    unique = []
    for p in fallback:
        if not is_phrase_used(p["english"]):
            p.setdefault("english_cyrillic", "")
            unique.append(p)
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 3} unique {category_english} phrases for Russian speakers learning English.
...
6. CRITICAL - FIELD RULES:
   - "english": ONLY English text (NO Russian words allowed!)
   - "russian": ONLY Russian translation
   - "english_cyrillic": The English phrase written with Russian/Cyrillic letters so that Russian speakers can read the pronunciation (e.g. "Dream big, start small" → "Дрим биг, старт смол")

Return as JSON array:
[{{"english": "English text", "russian": "Russian translation", "english_cyrillic": "English phrase in Cyrillic letters"}}]

EXAMPLES of CORRECT format:
✓ {{"english": "Dream big, start small", "russian": "Мечтайте по-крупному, начинайте с малого", "english_cyrillic": "Дрим биг, старт смол"}}
            ...
            if len(unique_phrases) >= num_phrases:
                ...
                for p in unique_phrases[:num_phrases]:
                    ...
                    normalized = {
                        ...
                        "english_cyrillic": p.get("english_cyrillic", ""),
                        "pronunciation": p.get("pronunciation", "")
                    }
                    if not russian_text:
                        print(f"[content] WARNING: Missing Russian text for '{normalized['english']}'")
                    if not normalized["english_cyrillic"]:
                        print(f"[content] WARNING: Missing english_cyrillic for '{normalized['english']}'")
...
async def generate_single_audio(text: str, voice: str, output_path: str, retries: int = 3):
    ...
    if "Dmitry" in voice:
        ...
        voices_to_try.append("en-US-JennyNeural")
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        if en_success:
            ...
            print(f"    ⚠ English TTS failed, generating {est:.1f}s silence")
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", str(est), str(english_file)]
            subprocess.run(cmd, capture_output=True)
        ...
        # Pause between Russian and English
        pause_between = 0.4
        total_duration = ru_duration + pause_between + en_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (RU: {ru_duration:.2f}s + pause: {pause_between}s + EN: {en_duration:.2f}s)")

        # Combine: Russian first, then English
        cmd = [
            "ffmpeg", "-y",
            "-i", str(russian_file),
            "-i", str(english_file),
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str, phrase_index: int = 0, total_phrases: int = 5):
    ...
    english_cyrillic = phrase_data.get("english_cyrillic", "")
    ...
    cyr_lines = wrap_text(english_cyrillic, font_cyrillic, max_text_w - 60) if english_cyrillic else []
...
def create_video_from_images_audio(image_files: list, audio_files: list, combined_audio: str, output_file: str):
    ...
    for i, (img_path, audio_info) in enumerate(zip(image_files, audio_files)):
        ...
        print(f"  Image {i+1}/{len(image_files)}: {duration:.2f}s (RU: {audio_info.get('ru_duration', 0):.1f}s + EN: {audio_info.get('en_duration', 0):.1f}s)")
...
def generate_reel(category_english: str = None):
    ...
    print(f"  🏷️  Branding: English with Kregg")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    print("  ✓ English with Kregg branding")
